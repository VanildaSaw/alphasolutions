...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    elseif(empty($_FILES['foto']['name'])) { $erro = 'A foto do produto é obrigatória.'; }
    else {
        $pasta = "../uploads/";
        ...
        if(!in_array($ext, ['jpg','jpeg','png','gif','webp'])){
            ...
            if(move_uploaded_file($_FILES['foto']['tmp_name'], $pasta.$novo_nome)){
                $stmt = $conn->prepare("INSERT INTO produtos (nome, preco, quantidade, descricao, foto) VALUES (?,?,?,?,?)");
                $stmt->bind_param("sdiss", $nome, $preco, $quantidade, $descricao, $novo_nome);
                if($stmt->execute()) $sucesso = true;
                else { $erro = 'Erro BD: '.$conn->error; unlink($pasta.$novo_nome); }
...
<title>AlphaSolutions - Adicionar Produto</title>
...
.card-header h3{font-family:'Rajdhani',sans-serif;font-size:13px;font-weight:700;
...
.field input,.field textarea{width:100%;padding:10px 14px;border:1px solid var(--border);
  border-radius:8px;background:var(--bg);font-family:'DM Sans',sans-serif;
  font-size:14px;color:var(--text);outline:none;transition:border-color 0.2s,box-shadow 0.2s;}
.field input:focus,.field textarea:focus{border-color:rgba(30,111,232,0.45);
  box-shadow:0 0 0 3px rgba(30,111,232,0.07);}
.field input::placeholder,.field textarea::placeholder{color:var(--muted);}
.field textarea{min-height:96px;resize:vertical;}
.input-prefix{display:flex;}
.input-prefix .pfx{padding:10px 12px;background:var(--border);
  border:1px solid var(--border);border-right:none;border-radius:8px 0 0 8px;
  font-size:11px;font-weight:700;color:var(--muted);display:flex;align-items:center;}
...
.upload-zone{border:2px dashed var(--border);border-radius:10px;background:var(--bg);
  text-align:center;padding:26px 20px;cursor:pointer;transition:all 0.2s;position:relative;}
.upload-zone:hover,.upload-zone.dragover{border-color:rgba(30,111,232,0.4);background:var(--light);}
.upload-zone input[type=file]{position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%;}
.upload-zone i{font-size:30px;color:var(--accent);opacity:0.45;display:block;margin-bottom:8px;}
...
.preview-wrap img{width:100%;max-height:190px;object-fit:contain;border-radius:8px;
  border:1px solid var(--border);background:var(--bg);padding:8px;}
.btn-remove-img{position:absolute;top:5px;right:5px;width:24px;height:24px;
  background:var(--red);color:white;border:none;border-radius:50%;font-size:11px;
  cursor:pointer;display:flex;align-items:center;justify-content:center;}
.file-label{margin-top:7px;font-size:12px;color:var(--green);
  display:none;align-items:center;gap:5px;}
.file-label i{font-size:13px;}

.msg-error{background:var(--red-light);border:1px solid rgba(239,68,68,0.2);
  color:#dc2626;padding:11px 14px;border-radius:8px;font-size:13px;
  margin-bottom:18px;display:flex;align-items:center;gap:8px;}
.btn-row{display:flex;gap:10px;flex-wrap:wrap;}
.btn-primary{padding:11px 26px;background:linear-gradient(135deg,var(--accent),var(--accent2));
  color:white;border:none;border-radius:8px;font-family:'Rajdhani',sans-serif;
...
.btn-ghost{padding:11px 18px;background:transparent;color:var(--muted);
  border:1px solid var(--border);border-radius:8px;font-family:'Rajdhani',sans-serif;
...
.s-icon{width:72px;height:72px;background:var(--green-light);border-radius:50%;
  display:flex;align-items:center;justify-content:center;margin-bottom:20px;
  animation:popIn 0.4s ease;}
.s-icon i{font-size:34px;color:var(--green);}
.success-screen h2{font-family:'Rajdhani',sans-serif;font-size:22px;font-weight:700;
  letter-spacing:2px;color:var(--green);margin-bottom:8px;}
.success-screen p{font-size:13px;color:var(--muted);margin-bottom:22px;text-align:center;}
.s-actions{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;}
...
<div class="success-screen">
  ...
  <p>O produto foi adicionado ao estoque com sucesso.</p>
  <div class="s-actions">
    <a href="adicionar.php" class="btn-primary"><i class="bi bi-plus-lg"></i> Adicionar outro</a>
    <a href="listar.php" class="btn-ghost"><i class="bi bi-list-ul"></i> Ver produtos</a>
  </div>
</div>
<?php else: ?>
...
<div class="main">
  <div class="page-top">
    <div class="page-title">Adicionar Produto
      <span>Preencha os dados para cadastrar um novo produto no sistema</span>
    </div>
  </div>
  ...
  <div class="form-card">
    ...
    <div class="form-body">
      ...
      <form method="POST" enctype="multipart/form-data" id="frmProd">

        <div class="field">
          <label><i class="bi bi-tag"></i> Nome do Produto</label>
          <input type="text" name="nome" placeholder="Ex: Notebook Dell Inspiron 15"
        ...
        <div class="grid-2">
          <div class="field" style="margin:0">
            ...
            <div class="input-prefix">
              ...
              <input type="number" name="preco" step="0.01" min="0.01" placeholder="0.00"
                value="<?php echo htmlspecialchars($_POST['preco'] ?? ''); ?>" required>
            </div>
          </div>
          <div class="field" style="margin:0">
            <label><i class="bi bi-stack"></i> Qtd. em Estoque</label>
            <input type="number" name="quantidade" min="0" placeholder="0"
        ...
        <div class="field">
          ...
          <textarea name="descricao" placeholder="Descreva as características do produto..."><?php echo htmlspecialchars($_POST['descricao'] ?? ''); ?></textarea>
        </div>
        ...
        <div class="field">
          ...
          <div class="upload-zone" id="uploadZone">
            ...
            <div id="uploadPH">
              ...
              <div class="upload-title">Clique ou arraste a imagem aqui</div>
              <div class="upload-sub">JPG · PNG · GIF · WEBP · máx. 5MB</div>
            </div>
          </div>
          <div class="preview-wrap" id="previewWrap">
            <img id="previewImg" src="#" alt="Preview">
            <button type="button" class="btn-remove-img" onclick="clearImg()" title="Remover"><i class="bi bi-x"></i></button>
          </div>
        ...
        <div class="btn-row">
          <button type="submit" class="btn-primary"><i class="bi bi-check-lg"></i> Salvar Produto</button>
